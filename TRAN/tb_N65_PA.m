function tb_N65_PA()
    close all
    res_dir = '/home/michael/simulation/tb_N65_PA/spectre/config/psf';
    res(1) = cds_srr(res_dir ,'tran-tran','v1');     
    res(2) = cds_srr(res_dir ,'tran-tran','vo1');     
    V1  = res(1).V*1e3;
    V2  = res(2).V*1e3;
    time = res(1).time*1e9;       
    plot_v(time, V1, 'PA PAD Input (5.8GHz)',0,2);
    plot_v(time, V2, 'PA PAD Output (5.8GHz)',0,2);
